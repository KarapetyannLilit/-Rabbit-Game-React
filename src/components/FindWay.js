export const nextCoordOfWolf = (source, target, board, forbiddenMoves) => {
  const possibleMoves = getAllPossibleDirections(source)
  const legalMoves = getLegalMoves(possibleMoves, board, forbiddenMoves)
  if (legalMoves.length) {
    return selectMinimumDistanceMove(legalMoves, target)
  }
  return source
}
